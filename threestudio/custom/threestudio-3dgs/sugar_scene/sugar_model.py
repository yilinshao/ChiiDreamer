...
class SuGaR(nn.Module):
    ...
    def __init__(
        ...
        *args, **kwargs) -> None:
        ...
        if surface_mesh_to_bind is not None:
            ...
            faces_colors = self._vertex_colors[self._surface_mesh_faces.long()]  # n_faces, 3, n_coords
    ...
    def quaternions(self):
        if not self.binded_to_surface_mesh:
            ...
            faces_verts = self._points[self._surface_mesh_faces.long()]
    ...
    def get_covariance(self, return_full_matrix=False, return_sqrt=False, inverse_scales=False, scaling_factor=None, enlarge_minaxis=None):
        if scaling_factor is None:
            ...
            _scaling = self.scaling.clone().detach()
            if scaling_factor == -1:
                scaling_factor = 0.85 / _scaling.max(1)[0].mean()
            _scaling *= scaling_factor
            if enlarge_minaxis is not None:
                if enlarge_minaxis == -1:
                    # enlarge_minaxis = (self.scaling.median(1)[0] / self.scaling.min(1)[0]).median() / 2.5
                    # enlarge_minaxis = max(enlarge_minaxis.item(), 1.)
                    enlarge_minaxis = 0.09 / _scaling.min(1)[0].mean()
                min_scale_axis = _scaling.min(1)[1]
                _scaling[torch.arange(_scaling.shape[0]), min_scale_axis] *= enlarge_minaxis
            scaling = _scaling
            scaling = torch.ones_like(_scaling)*5
    ...
    def save_ply(self, checkpoint_path='.', iteration=0):
        ...
        def construct_list_of_attributes():
            ...
            for i in range(self._sh_coordinates_rest.shape[1] * self._sh_coordinates_rest.shape[2]):
    ...
    def marching_cubes(self, iteration=0, checkpoint_path='.', resolution=256, vertex_color=False, thres=0., bound_min=None, bound_max=None):
        bound_min = - 1.3 * np.ones(3) if bound_min is None else bound_min
        bound_max = 1.3 * np.ones(3) if bound_max is None else bound_max
        N = 64
                ...
                for yi, ys in enumerate(Y):
                    for zi, zs in enumerate(Z):
                        ...
                        pts = torch.cat([xx.reshape(-1, 1), yy.reshape(-1, 1), zz.reshape(-1, 1)],
                                        dim=-1).cuda()  # [N,3]
                        sample_sdf = self.neus.sdf_network.sdf(pts)  # [N]
        ...
        if vertex_color:
            ...
            for pts in torch.tensor(vertices,dtype=torch.float).cuda().split(50000):
                with torch.enable_grad():
                    normals = self.neus.sdf_network.gradient(pts).squeeze()
        ...
        mesh.export(os.path.join(debug_path, 'mcubes_' + str(iteration) + '.ply'))
    ...
    def visual_moved_point_cloud(self, iteration=0, checkpoint_path='.'):
        ...
        moved = []
        with torch.enable_grad():
            for pts in self.points.split(50000):
                positions_grad = self.neus.sdf_network.gradient(pts).squeeze()
                sdf = self.neus.sdf_network.sdf(pts)
    ...
    def validate_image(self, image, camera_indices=0, iteration=0, checkpoint_path='.'):
        if image is not None:
            ...
            outdir = os.path.join(checkpoint_path, 'render')
            os.makedirs(outdir, exist_ok=True)
            img_name = f'{iteration:06d}_{camera_indices}.jpg'
            cv2.imwrite(os.path.join(outdir, img_name), image[..., [2, 1, 0]])

    def validate_normal_image(self, camera_indices=0, iteration=0, checkpoint_path='.'):
        nerf_cameras = self.nerfmodel.training_cameras
        ...
        point_normals = self.get_normals()
        ndc_coords = torch.tensor([[0, 0, -1.0]]).cuda()
        world_coords = p3d_camera.unproject_points(ndc_coords, world_coordinates=True)
        camera_center = p3d_camera.get_camera_center()
        ray_directions = world_coords - camera_center
        ray_directions = ray_directions / torch.norm(ray_directions, dim=-1, keepdim=True)
        splat_colors = point_normals * torch.sign((point_normals * ray_directions).sum(dim=-1, keepdim=True))
        ...
        outdir = os.path.join(checkpoint_path, 'render')
        os.makedirs(outdir, exist_ok=True)
        img_name = f'{iteration:06d}_normal_{camera_indices}.jpg'
        cv2.imwrite(os.path.join(outdir, img_name), image[..., [2, 1, 0]])
