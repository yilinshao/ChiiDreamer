...
@dataclass
class RandomRotateCameraDataModuleConfig:
...
class RandomRotateCameraIterableDataset(IterableDataset, Updateable):
    def __init__(self, cfg: Any) -> None:
        super().__init__()
        self.cfg: RandomRotateCameraDataModuleConfig = cfg
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        # ========== Starting from, camera intrinsics and extrinsics ========== #
        ...
        focal_length: Float[Tensor, "B"] = 0.5 * self.height / torch.tan(0.5 * fovy) # tick
...
class RandomRotateCameraDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        # ======================================================== #
        # ==== edited for rotate around a single center point ==== #
        camera_positions: Float[Tensor, "B 3"] = torch.stack(
            [
...
@register("random-rotate-camera-datamodule")
class RandomRotateCameraDataModule(pl.LightningDataModule):
    cfg: RandomRotateCameraDataModuleConfig

    def __init__(self, cfg: Optional[Union[dict, DictConfig]] = None) -> None:
        super().__init__()
        self.cfg = parse_structured(RandomRotateCameraDataModuleConfig, cfg)

    def setup(self, stage=None) -> None:
        if stage in [None, "fit"]:
            self.train_dataset = RandomRotateCameraIterableDataset(self.cfg)
        if stage in [None, "fit", "validate"]:
            self.val_dataset = RandomRotateCameraDataset(self.cfg, "val")
        if stage in [None, "test", "predict"]:
            self.test_dataset = RandomRotateCameraDataset(self.cfg, "test")
