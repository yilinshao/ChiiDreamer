...
if __name__ == '__main__':
    ...
    image = pipe(prompt_final, bboxes, num_inference_steps=50, guidance_scale=7.5, 
                    MIGCsteps=25, aug_phase_with_and=False, negative_prompt=negative_prompt, sa_preserve=True).images[0]
    
    image.save('before.png')
    image = pipe.draw_box_desc(image, bboxes[0], prompt_final[0][1:])
    image.save('anno_before.png')
    
    
    prompt_final = [['masterpiece, best quality,black colored ball,yellow colored dog,white colored bed,\
                     green colored plant,red colored teddy bear,blue colored wall,brown colored vase,orange colored book,\
                     yellow colored hat', 'black colored ball', 'yellow colored dog', 'white colored bed', 'green colored plant', \
                        'red colored teddy bear', 'blue colored wall', 'brown colored vase', 'orange colored book', 'yellow colored hat']]
    
    seed_everything(seed)
    image = pipe(prompt_final, bboxes, num_inference_steps=50, guidance_scale=7.5, 
                    MIGCsteps=25, aug_phase_with_and=False, negative_prompt=negative_prompt, sa_preserve=True, use_sa_preserve=True).images[0]
    
    image.save('modify_cat_to_dog.png')
    image = pipe.draw_box_desc(image, bboxes[0], prompt_final[0][1:])
    image.save('anno_modify_cat_to_dog.png')
    
    
    prompt_final = [['masterpiece, best quality, black colored sword, frontal view, white background', 'black colored sword']]
    
    bboxes = [[[0.3125, 0.609375, 0.625, 0.875]]]
    
    seed_everything(seed)
    image = pipe(prompt_final, bboxes, num_inference_steps=50, guidance_scale=7.5, 
                    MIGCsteps=25, aug_phase_with_and=False, negative_prompt=negative_prompt, sa_preserve=True, use_sa_preserve=True).images[0]
    
    image.save('remove_hat.png')
    image = pipe.draw_box_desc(image, bboxes[0], prompt_final[0][1:])
    image.save('anno_remove_hat.png')